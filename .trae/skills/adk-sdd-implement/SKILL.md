---
name: adk-sdd-implement
description: "Use when the user wants to execute `tasks.md`, implement approved SDD tasks in order, update task progress, and verify results; do not use for ordinary coding requests that are not task-list driven."
---

# ADK SDD Implement

This skill is an SDD stage skill for implementing work according to a task list. It is suitable for executing work in `tasks.md` order, writing back status, and completing validation.

## Manual Trigger Constraints

- Use this skill only when the user explicitly wants to implement work according to `tasks.md`, update task status, and perform completion validation.
- Do not automatically route ordinary coding requests to `adk-sdd-implement`.
