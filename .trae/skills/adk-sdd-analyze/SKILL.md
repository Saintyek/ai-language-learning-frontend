---
name: adk-sdd-analyze
description: "Use when the user wants a read-only review of `spec.md`, `plan.md`, and `tasks.md` for consistency, coverage, ambiguity, or constitution alignment; do not use when they want files changed."
---

# ADK SDD Analyze

This skill is a read-only quality analysis helper in SDD. It is suitable for checking whether `spec.md`, `plan.md`, and `tasks.md` are aligned and for identifying gaps.

## Manual Trigger Constraints

- This is a read-only analysis skill and must not modify any files.
- Use this skill only when the user explicitly wants to check consistency, coverage, ambiguity, or alignment with the constitution.
...
Run `node .ttadk/plugins/ttadk/core/resources/scripts/check-prerequisites.js --json --require-tasks --include-tasks` once from repo root and parse JSON for FEATURE_DIR and AVAILABLE_DOCS. Derive absolute paths:
